...
class QTrain:
  ...
  def readCSVFile (self):
    with open("QTable.csv", 'r') as f:
      self.qtable = np.loadtxt(f, delimeter=',')
  ...
  def chooseMove (self):
    ...
    success, new_state_index = self.GameBoard.actionToFunction[actions[action]](coords)
  ...
  def train(self):
    for episode in range(episodes):
      ...
      for step in range(steps_per_game):
        ...
        self.qtable[state, action] = (1-learning_rate)*self.qtable[state, action] + learning_rate*(reward + discount_rate * np.max(self.qtable[new_state, :]))
        self.updateCSVFile()
    ...
    self.exploration_rate = min_exploration_rate + (max_exploration_rate-min_exploration_rate) * np.exp(-exploration_decay_rate*episode)
...
def zombieMove(self):
  ...
  if optimum_state != False:
    ...
    if action == "bite":
        ...
        move_coord = self.GameBoard.toCoord(optimum_state.location)
        self.GameBoard.actionToFunction[action](move_coord, prev_state.person.zombieStage)
        return True # bitten
    else:
